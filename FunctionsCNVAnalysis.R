...
###FUNCTION INDEX ########
# auto.mode
# is.ch is function for character() or list of characters
# extend.50pc
# update.cnvrs.with.cn
# penn.name.files PennCNV commands are more easily run using files listing the locations of each sample file. This function generates a set of such files using relative or absolute paths, given a specified number of directories
# import.called.cnvs
# get.baf.markers attach datafile bigmemory object
# do.scree.plots plumbCNV wrapper to do SCREE PLOTS AND calculate EIGENVALUE VARIANCE after a PCA
# run.PCA.correct Run PCA on a subset of the combined dataset then correct the combined dataset according to the first 'n' PC-components then stats on the resulting distributions can be calculated write results to big.matrix files and pdfs, etc, then returns raw, sample-QC and PC-corrected sets of stats for comparison
# load.data.to.bigmat get data stored in a text or binary file and get it into a bigmatrix object format
# post.pc.comparison compare pre-pc and post-pc stats; might want to pass sample.info in for consistency
# lrr.sample.dists calculates sample distributions for mean, optionally DLRS/StDev, GC-wave. passes arguments to calc.chr.info via get.chr.info.filt and calculate.gc.for.sample better off passing a complete snp.info object, as will filter out any not in the data automatically lo.cut=c("LB",NA,"LB"),hi.cut=c("UB","UB","UB") are which cutoff to use for the columns of stat.table which are generally Mean, DLRS and GCWave respectively, although if DLRS or GCwave are turned off, then this can change load.all.libs() load all main plumbcnv libraries
# get.excl.filter.for create sample-wise filter for any object, excluding samples in exclusion files directory
# exclude.combine.big.mats apply exclusions from sample QC and combine all matrices from different datasets into a single big.matrix object
# load.big.pointer.list taking a list of big matrix descriptors (of some sort) return list of pointers to each big.matrix, plus information about the comprised samples
# do.quick.LRR.snp.assocs use sample.info create list of bigMatrix locations go 'N' snps at time, concatenate into 1 file, run regression assocs for 2 phenotypes/grps gives t values - ordinally equivalent to logistic regression with 2 groups
# snp.select.least.assoc association version of selecting subset of snps for PCA takes the 'pc.to.keep'% least associated with sample.info$phenotype'
# extract.snp.subset extract small percentage of SNPs as a subset to commit to PCA to correct batch effects
# batch.effects.density.plot detailed density plot for each batch (e.g, plate) for mean and DLRS distributions
# lrr.boundary.scatter makes scatterplot for each combination of stats versus each other requires 'plot.stats'; also uses 'pass.stats' which is the reference (may be the same as plot.stats)
# get.plate.lrr.stats table of plate-wise LRR stats
# dual.cat simultaneously apply the cat function to the screen and an open connection
# rv.large.to.file print a matrix to file nicely
# make.QC.summary.table create exclusion comparison table show overlap between samples excluded by various QC metrics use just annotated exclusion lists
# LRR.boxplot make boxplot of LRR for each statistic in 'stats.table'; if file name present to file, else to screen; add appropriate labels
# calc.main.stats.on.big do the calculation of columnwise Mean, stdev and DLRS(optionally) for a big.matrix
# big.matrix.operation apply simple arithmetic functions to a pair of big matrices splits the operation into 'split.to' sets of rows. only need to increase this if the datafile is more than 20 times the available RAM memory.
# do.venn.QC for up to 3 columns of sample statistic pass/failures in a table, do venn diagram
# trim.plate.duplicates remove duplicate entries in a plate.lookup index dataframe that was generated by 'get.plate.info'
# get.plate.from.sample.info extract plate.info object from a sample info object
# get.plate.info get a table of plate information from a plate lookup file. fairly flexible but lookup must contain plate ids alongside sample ids as a minimum requirement. may include additional information such as 'well'
# excl.bad.plates get list of plates with more than 'badPlateThresh' percent of samples failing on at least one QC measure. return list of plates, and samples in these plates, by default write the sample information to an exclusion file 'BadPlates.txt'
# chr.ab.report make chromosomal abberation graphs, write sample exclusion file 'ChrAb.txt' to dir$ano
# get.chr.stats get the LRR mean and SD for each chromosome (autosome) separately used for detection of chromosomal abberations
# make.chr.fail.table using chromosome (autosome) means from 'get.chr.stats', create list of failures per sample for their chr.mean to within confidence limits
# do.chr.ab.contrasts Set up contrast matrices to test for chromosomal aberrations (autosomes) two matrices, one a simple average comparison, target vs rest, the other a local comparison, target versus adjacent weighted 2^n generate local chromosomal contrast matrix comparison of target chromosome to weighted mean of adjacent chromosomes
# do.med.chunk internal function for do.median.for.ranges() to calculate contents of 1 loop
# do.median.for.ranges the diskin method to calculate GC requires sample-wise medians for snps in each 1MB window of the genome
# get.ranges.for.median get list of position ranges for snps in each 1MB window of the genome, ignoring any window with less than 'min.size' snps
# remove.qc.failers use column 'QCfail' in sample.info to remove failing ids from any frame sample.info could be any dataframe where rownames are sample ids. dat <- force.frame(tab) allows flexible input, forces dataframe result
# samp.update.qc.fail
# snp.update.qc.fail
# sync.samp.info.with.file.spec
# validate.samp.info make sure current sample.info object conforms to the expected structure update QC failure flags based on sample exclusion directory update 'grp' membership based on file.spec.txt column 'GRP'
# get.subIDs if separate id lists for each cohort are in 'dir' then create list of these
# get.file.specs check for and read specifications file 'file.spec.txt' from directory dir$lrr.dat input as 'dir' list or otherwise will try to convert to this format from character()
# get.file.lens read in file lengths from text file 'file.lengths.txt' (generated during bash import)
...
# get.lrr.files wrapper for get.lrr.dat.file.names
# get.lrr.dat.file.names tries really hard to find the right datafiles to read in for LRR raw data
# time.est.snps calculates the expected number of time to import a number of lines into snpMatrix format
# get.snp.read.fn if read.snps.long is ever removed (function is currently deprecated) then this wrapper helps suck up redundant variables and redirect relevant ones to 'read.long'
# import.snp.matrix.list Reads in the SNP data for the whole dataset, with a separate snpMatrix entry per cohort setting HD=T uses a lot less RAM, can be used for very large datasets (but slow) option multi allows use of parallel processing, but the main limited is hard disk speed, so for now this is likely to decrease performance if anything.
# draw.density.plots draw density plots for SNP/sample-wise callrate evaluation
# update.list.with.list idea is start with a full list, and if any elements with the same name are in add.list, and they are the same type, then the original list entry will be updated with the add.list entry. the idea is to facilitate easy control of a large number of parameters (which have defaults) while only having to specify those to change, not all of them
# get.grp.level.snp.stats do some snp-qc on each group separately and look for between-grp differences
# get.hdr.lens get the lengths of headers in genome studio files assuming the header must be less than 'feas.hdr'/500 lines long. starts from line 500 counts the number of tabs/separators and looks for when the number first changes. if it doesn't assumes 1st row only is header if firstrowhdr = T, else assume no headers at all
# load.all.libs loads most of the libraries required by plumbcnv in a fairly discreet manner note that lattice uses parallel function 'parallel' so should be loaded second because plumbCNV always refers to parallel::mcparallel to prevent confusion
# preload.bioC load bioconductor packages without annoying package loading text bio.packs <- c(bio.packs,more.bio) allows adding to, rather than replacing default set
# correct.gc.old apply PC correction for a single SNP, allowing for missing data.
# correct.gc.mat.old matrix version of correct.gc (used to GC-correct one SNP at a time)
# lrr.stats.tab CALCULATE LRR STATS based on a table of sample-wise LRR stats (eg, mean, stdev, dlrs, etc) calculate distribution indices for each LRR-sample-statistic for the whole cohort if(!is.numeric(nSD)) { nSD <- 3 }; nSD <- abs(nSD) ensure positive and numeric
# plot.extreme.samples plot the most extreme sample for each combination of QC failure types
# get.all.samp.fails.old get all sample ids in fail lists in dir.ano/SAMPLE_EXCLUDE/
# get.chr.ab.fail.subset SCRIPT TAKES LIST OF SAMPLES WITH CHR ABERRATIONS (using contrasts) AND SELECTS A SUBSET TO FLAG/PLOT WHICH CAN BE BASED ON NOT FAILING OTHER CRITERIA
# insert.fake.median a hack for now to avoid fixing code so Median can be left out of tables
# add.chr.top.axis on chromosome graphs there is an option in plumbCNV to have the chromosomes on the top axis, this script manages this allowing for different levels of zoom which may cause overlaps. manages it better than the default. also can change a target chr label to red colour.
# bw.plot.lrr Black and White Type Plot Alternate graph without the full LRR data This plots the means of each chromosome and their confidence intervals also overlays the mean and std deviation bands of the reference group of all good samples CI.Z = confidence interval z score
# plot.chr.ab.samples PLOT IN ONE OF TWO WAYS (BW or Color) plot all bad samples in 'checklist'
# stdev
# choose.best.sort.file hopefully sort directory only contains 1 valid file if there are multiple files, no files, or only invalid files with no matches to ref.list (i.e, to the current big.matrix) this script will try to extract/create the best possible list
# check.file.and.subset check that file dir/fnm is a genuine subset of the row/col names (by.row) of matrix 'mat' return subset or if not valid, then return all row/col names of the original 'mat'
# sort.exclude.from.annot sort big matrix according to annotation and exclude those in exclusion files
# select.samp.snp.custom based on files/vectors of snp-ids and sample-ids create selection vectors to select only the ids in these lists for a matrix
# big.exclude.sort sort and exclude snps/samples from a big.matrix
# thresh.check.valid print("nSD");print(nSD) CHECK VALIDITY OF CHOSEN Mean, DLRS and GC THRESHOLDS return validated set, or just T/F if check.only=T
# get.excl.list.from.stats calculate which samples in stat.dat should be excluded based on statistical thresholds on the LRR data in stat.frame CALC EXCL SAMPLES
# get.fail.type.list GET LISTS OF SUBJECTS FAILING ON ALL POSSIBLE SETS OF CRITERIA USE FOR INDIVIDUAL GENOME vs LRR Plot Examples
# remove.boundary.snps.window remove snps which when adding a window exceed the boundaries of their chromosome e.g, for analyses with windows calculations, e.g 1MB either side of each snp, etc
# basic.cnv.counts do some basic counting for overlaps between a CNV list and a reference mainly used as a subfunction of count.cnvs()
# sample.cnv.counts do some basic counting for overlaps between a CNV list and a reference with respect to samples mainly used as a subfunction of count.cnvs()
# add.color.to.snp.info add colour scheme for plotting cnv/snp data mono = all col1, alternate = alternate between col1 and col2, unique = all different, hilite all col2 except 'hilite' chrm(s)
# add.gindx.to.Ranges
# filter.snp.info select(include=T) or remove(F) set of 'filt.names' from snp.info
# get.chr.info.filt calculate chromosome indexes, in particular for the filtered version of snplist rownames(bigMat2) basically does nothing if recalcF == FALSE
# sample.bad.count.table look at all sample exclusion files and make table of how many excluded for each reason for each plate
# list.qc.fail.types look at all sample exclusion files and make table of how many excluded for each reason
# update.plate.bad.count.table look at all sample exclusion files and make table of how many excluded for each reason for each plate plate.index should contain sample ids in column 1, plate ids in column 2 would be easy to run for wells by switching cols 2&3
# do.lens.summary prints a simple summary of an inputted vector of CNV lengths
# force.pheno.codes try to cleverly assign phenotype codes
# force.sex.codes try to cleverly assign sex codes
# read.ped.file
# get.trios
# make.trio.file
# make.fam.file create plink family file using sample.info Family ID,Sample ID,Paternal ID,Maternal ID, Sex (1=male; 2=female; other=unknown,"\n") Affection (0=unknown; 1=unaffected; 2=affected,"\n")
# filter.hifreq.plates filter samples with too many rare DELs/DUPs (according to specified rare rates) (poisson) or filter samples with too many CNVs (normal)
# filter.hifreq.cnv.samples filter samples with too many rare DELs/DUPs (according to specified rare rates) (poisson) or filter samples with too many CNVs (normal)
# change.dir.data.tracker if wanting to change the directory for a session, you must edit the directory in many parts.
# blank.data.tracker setup a blank data.tracker object, optionally with some directory info
# init.data.tracker Initialise a new DATATRACKER object
# setSlot update some field(s) in a datatracker object put filenames etc into groups according to parameter 'grps' if 'grps' not inputted, use other information to infer if shape.lrr is inputted only works if input is sorted by grp e.g, 1,1,2,3,3,3,3,4, etc
# methods.data.tracker
# req.valid ensure that required fields are present and valid in a datatracker object
# three.way.comparison function assuming data is PC-clean and raw stats per sample summary files are in the default location with default names. if so, makes nice visualisation
# combine.raw.stats combine set of sample statistics from multiple files (cohorts) into 1 big file or in the case of length grpnumz = 1, simply return the only file. can deduce in 3 ways: i) user specified in run.PCA.correct; ii) from DT; iii) look for expected number of grps
# batch.box.plots plot box plot of batch effect, using stats summary and looking up batch categories from table
# get.extreme.examples find most extreme subject for each part of venn list [NB: or for 'none' group - the least extreme ]
# col.extremes if ="extr" then colour extreme values in a (pre) latex object otherwise colour red/blue according to logical vars selBlue, selRed
# initialise.excl.files delete existing sample exclusion file if present
# gs.bash.http load package
# force.good.plink
# init.DATA.read
# run.SNP.qc load in all the SNP-data need to do SNP-QC to exclude bad snps. use snpStats functions to get callrates, etc, else can use plink results processed at an earlier stage during 'getRawData.R' or 'getDataGS.sh' execution. do SNP-qc and export lists of excluded samples, build sample.info and snp.info objects load.all.libs() load all main plumbcnv libraries
# do.chromosomal.abberations detect, flag and plot samples with chromosomal abberations better off passing a complete snp.info object, as will filter out any not in the data automatically if skip.chr.ab = T then just write a blank exclusion file and continue
# do.plate.qc PLATE STATS
# process.cohort.qc loop through once for each file passes arguments to lrr.sample.dists which passes to calc.chr.info better off passing a complete snp.info object, as subfunctions will filter out any not in the data automatically
# run.SAMPLE.qc run the whole sample QC process from scratch including importing raw data load.all.libs() load all main plumbcnv libraries check validity of thresholds before proceeding and warn so changes can be made before time wasted
# make.grp.column
# dat.to.big.matrix import from a text (hopefully long format) datafile to a big.matrix return bigmatrix description 'object' or name of description file according to 'ret.obj' CALIBRATE OPTIONS AND PERFORM CHECKS max.mem <- 4096 stupidly large bafmode <- F assume in LRR mode unless otherwise indicated by 'pref'
# make.dir create a directory structure for plumbCNV to work upon starting with a rawdata and base directory raw data: dir.raw <- "/ipswich/data/Immunochip/FinalReports/" location of support files directory dir.sup <- "/chiswick/data/store/metabochip/PLINK/" location to write summaries of call rate: dir.base <- "/chiswick/data/ncooper/ImmunochipReplication/" make sure each ends in a slash dir.force.slash <- reader:::dir.force.slash use internal function from 'reader'
# lrr.dat.to.big.matrix converts set of LRR long format/matrix *.dat files into big.matrix objects
# baf.dat.to.big.matrix converts a BAF long format/matrix *.dat file(s) into a big.matrix object
# import.DATA.big If using the datatracker, allows import of baf and lrr shaped data to big matrix format with minimal input requirements.
# confirm.corrected.filename expects PC corrected bigmatrix to have a file name of a certain format, given the number of PCs corrected for. If this expected filename is not found, then it will look for the nearest thing in the relevant directory
# prepare.one.sample all the commands to process a single sample file. mainly packaged into a function to tidy up the code in the main function
# prepare.penncnv.samples penn-cnv requires the LRR and BAF data for each sample to be in a separate file this function imports the BAF data if not present, then writes individual LRR,BAF files for all samples
# prepare.penncnv.markers penn-cnv requires a pfb file and a gcm file to run. These show the average BAF and the average GC percentage for every marker, as well as the locations and labels
# get.trios.cmd this function generates calls to runn penn-cnv for 'ndir' directories created by plumbcnv can run with either absolute or relative file paths. marker.fn is a list containing elements 'baf' and 'gc' which are the files from prepare.penncnv.markers()
# get.penn.cmd this function generates calls to runn penn-cnv for 'ndir' directories created by plumbcnv can run with either absolute or relative file paths. marker.fn is a list containing elements 'baf' and 'gc' which are the files from prepare.penncnv.markers()
# process.penn.results This function runs the penn cnv output through a series of filters from penn scripts, plink commands, both utilising bash commands, and also R-functions, to end up with the final QC-ed set of deletions, duplications, CNVs, both common and rare.
# dir.force.slash make sure 'dir' directory specification ends in a / character
# q.submit make the sh file make the qsub call submit to queue
# q.test.done check the flag files and return the numbers of any completed
# do.penn.qsub
# bash.qsub
# run.PENN.trios validate CNVs with trios if we have family data takes roughly as long as the original penn-cnv analysis only where the data is in trios of child-father-mother, runs from 6-column plink ped/fam file
# run.PENN.cnv takes PC corrected data, generates the prerequisite PENN-CNV input files and runs PennCNV to detect CNVs (or gives the commands to run manually) will import and process the BAF data/file during this process load.all.libs() load all main plumbcnv libraries
# run.CNV.qc takes PC corrected data, generates the prerequisite PENN-CNV input files and runs PennCNV to detect CNVs (or gives the commands to run manually) will import and process the BAF data/file during this process
# extract.val.penn penn cnv files have format varname=var.value; extract just the value
# count.penn.dirs looks at list of files in dir$cnv.raw and finds the highest suffix p-xx and assumes this corresponds to the number of penn-cnv directories
# remove.for.step helps reduce file space by deleting large files no longer needed from previous steps of the pipeline. can delete after complete step 'n'
# delete.big.matrix.files delete the backing, description and optionally 'RData' file for a specific bigMatrix. RData only deleted if bigMat is the name of this file
# delete.file.list delete a list of files
# delete.all.files
# do.roc
# qscore.cnvs
# ratio.flanks
# get.ratio.set
# big.extract.snp.ranges
# prob.norm
# get.flanks.from.big.mat
# kf.dist.test
# cnv.sanity.test
# t.kf.test
# t.kf.test.fix
# test.lrr.as
# most.likely.copy returns most likely copy number for a baf sample, with confidence versus next best option if using test=x, this tests a specific copy-x against the most likely alternative
# copy.state.baf.lik
# copy.state.lrr.lik
# est.state.priors if it 's in the cnv then likely to be that copy state near cnv only used when in.cnv =F; in.cnv set to F if cnv.copy=2
# lrr.by.copy the expected LRR mean for each copy number
# read.penn.hmm.pars
# calc.quality.stats get the raw data
# one.cnv.report
# get.quality.scores
# make.qs.table
# boundary.stats return all stats, or potential lower/upper bounds lower/upper bounds go from least extreme to most extreme (least deleterious) cutoff
# coverage.change report on loss of genome coverage due to QC excluded snps
# write.snp.info
# write.data.tracker
# read.snp.info
# read.data.tracker
# write.sample.info write standard 'sample.info' object to file
# read.sample.info read standard 'sample.info' object from file
# get.PCA.subset extract LRR matrix with subset of SNPs, ready for PCA analysis
# random.spacing.snp.select this assumes roughly whole genome coverage. breaks down if this is not roughly true uses the chr, pos, label of each snp, stored in a genoset RangedData object to yield a % (e.g, 5%) subset of snps evenly spaced throughout the genome
# print.snp.sample.summary make simple summary of number of passing/failing SNPs at different processing stages
# calculate.gc.for.samples calculate GC-wave stats for each sample
# make.sample.info create sample info dataframe, combining an id list with phenotype, plate, group, or any other data, inputted as a named list of objects or file names.
# add.to.sample.info adds columns 'to.add' or else all valid columns from table 'more.info' to the object sample.info, for matching rownames. autodetects id column in more.info based on the rownames of sample.info. can optionally overwrite existing columns
# samp.cr.summary calculate call rate using percentage of missing by column, in a bigmatrix
# snp.cr.summary calculate call rate using percentage of missing by column, in a bigmatrix
# range.fin
# length.analysis.pic
# plot.one.la
# plot.one.errbar
# length.analysis.suf
# length.analysis
# count.genes genes in my functions can be stored in a single field separated by semicolons this uses number of ;'s plus 1 to count genes
# power.analysis.fet bu = basic unit of table, rng = max ratio szs <- c(0.5,0.8,(1:10))*bu dataset sizes to test
# power.analysis.fet2 bu = basic unit of table, rng = max ratio szs <- c(0.5,0.8,(1:10))*bu dataset sizes to test
# print.biggest.cnvs
# LRR.gc.correct using GC%, run correction for GC-wave on a dataset
# get.gc.markers for a list of SNPs, get the average GC% in the 1MB surrounding window load annotation package from bioconductor
# get.gc.human get the mean GC% for every 1MB window of the human genome can set build build ('build') to hg18 or hg19 print(paste("buildThere",build))
# calcGC taken from genoset package
# get.all.snp.fails get all sample ids in fail lists in dir.ano/SAMPLE_EXCLUDE/
# get.all.samp.fails get all sample ids in fail lists in dir.ano/SAMPLE_EXCLUDE/
# plot.all.ranges
# get.tracker.for.plot hidden function just for cnv.plot and plot.all.ranges
# get.pfb.ranges
# get.snpqc.ranges
# cnv.plot customLims <- rngOn <- zoom <- F change this later if valid limits received
# MedianScale smooth data series by median / mean
# get.dat.for.ranges based on ranges object, provide a plumbCNV directory and choose whether LRR or BAF, and if LRR, whether pre/post pc-correction also choose chr/genome coords, whether use snp.names as labels, and whether to return in x,y format with snp positions.
# x.y.for.snp.range for a frame of first and last snps (fls), extract these ranges from bigMat, either in genome coords, or just in chr-relative coords
# col.plot.lrr Color Type Plot colour type plot of LRR data for a sample over a given range, optional smoothing, overlay of means, etc. print(head(snp.info)) targ.chr.ref sets the x axis scale start point (e.g., relative to start of what chromosome)
# dlrs calculate dlrs
# gene.duplicate.report for a RangedData object, report on any multiple listings for the same gene
# make.genes.unique for a RangedData object, convert any multiple listings for the same gene into one entry, merging ranges to min,max
# dgv.subset.cov.by.n.snps return the subset of the DGV covered on the current chip by 'n.snps', or if 'add.col' then just adds a snp count column to dgv dataset
# count.cnvs uses result from 'find.overlaps()' a function to count the number of CNVs, genes, exon, etc. sapply? number of unique gene/ex/dgv per CNV; number of gene/ex/dgv hits across all CNVs; number of unique gene/ex/dgv across all CNVs all these could have percentage filter applied relative to reference or source
# do.CNV.all.overlaps.summary pass the results list from plumbCNV() and include any args of 'find.overlaps()' for filtering
# summary.counts.table do a table for overlap with each database at a time columns are type of CNV, rows are type of count prv(CNV.overlaps.summary) start local function
# pheno.ratios.table
# annot.cnv which genes overlap with each CNV - don't forget to account for the empty ones!
# getSlot retrieve a field from a data tracker object (using short labels), can also do for grp subset list valid values for 'what' by type of file they refer to
# is.data.tracker check whether object is in correct 'data.tracker' format
# make.snp.info make a snp.info object from scratch by combining a list of snps (in order of datafile) with an annotation file (map3/bim/vcf/etc), optionally filtering for only 'filt.by' snps
# check.data.size
# q.cmd
# make.bad.region.file
# add.info.to.ranges add information to a RangedData object (like phenotype) min.chars <- 4 column name in 'file' must have at least the first 'min.chars' letters of 'target'
# plot.pheno.cnvs make plot of CNV regions across chromosomes in competing phenotypes for rare dels/dups
# init.dirs.fn create the directory structure implied by the object 'dir' provides options to delete existing directories if overwrite=T
# check.readiness check whether all appropriate files are in place to run plumbCNV Check for appropriate Penn CNV installation mode 1: run the pipeline starting from a raw genome studio file, using file.spec.txt mode 2: run the pipeline starting from pre-formatted LRR/BAF longfiles or matrices mode 3: run the pipeline starting with big matrix object(s) snp mode 1: run SNP QC using SNP STATS, must have genotype data snp mode 2: skip SNP QC step (e.g, may have already excluded low quality snps) snp mode 3: run SNP QC using plink (if from scratch) or have already run SNP-QC in plink manually
# get.plate.info.for.big
# get.plate.snp.stats.for.big
# plumbCNV run the entire plumbCNV pipeline with mostly default filenames.
# plumbcnv
######## END FUNCTION INDEX #########
...
update.cnvrs.with.cn <- function(cnv.list,double.table,DEL=TRUE) {  
  ...
  for (dd in 1:nrow(double.table)) { 
    ii <- subsetByOverlaps(as(cnv.list,"GRanges"),as(double.table[dd,],"GRanges"))
    if(nrow(ii)>0) {
...
get.baf.markers <- function(baf.des.fn, samp.list, dir, low.ram=T) {
  ...
  bigBAF <- get.big.matrix(baf.des.fn,dir=dir$big)
...
# print(dir[[1]]) ; 
  ...
  if(num.pcs<1) {
    ...
    mind <- min(Dim(get.big.matrix(subset.descr,dir=dir$big)))
  ...
  corrected.bigMat <- get.big.matrix(corrected.ref,dir=dir) #; rm(corrected.ref) #(in case it was a bigmat)
...
exclude.combine.big.mats <- function(bigList,dir="",pref=paste("SampQC_Combined",sep=""),debug=F) {
  ...
  if(length(bigList)==1)
  {
    bigUnified <- get.big.matrix(big.exclude.sort(bigList[[1]], dir=dir, pref=pref),dir=dir$big,verbose=F)
...
load.big.pointer.list <- function(descr.list,dir)
{
  ...
  { 
    bigMatLst[[cc]] <- get.big.matrix(descr.list[[cc]],dir=dir) 
...
do.quick.LRR.snp.assocs <- function(descr.fn,sample.info=NULL,use.col="phenotype",dir,p.values=T,n.cores=1)
{
  ...
  bigMat <- get.big.matrix(descr.fn,dir=dir)
                                   ...
                                   snp.info=NULL,n.cores=1,apply.method=T,rmv.chr.bounds=T) 
{
  ...
  # attach datafile bigmemory object
  bigMat2 <- get.big.matrix(des.fn,dir=dir)
...
get.chr.stats <- function(bigMat,snp.info,dir="",allow.subset=F)
{
 ...
 if (allow.subset) { 
   if(nrow(snp.info)!=nrow(bigMat)) {
     ...
     rm(bigMat); bigMat <- get.big.matrix(big.descr,dir=dir$big)
   }
 } else {
...
get.subIDs <- function(dir,ret=c("all","lists","combined","files","groups"),verbose=F,combine.grp=F)
{
  ...
  if(!all(file.exists(path.fn))) {
    faild <- FALSE
    if(num.filz==1) {
      all <- cat.path(dir$ids,"subIdsALL.txt")
      if(file.exists(all)) { system(paste0("cp ",all," ",id.fnz[1])) } else { faild <- TRUE }
    } else { faild <- TRUE }
    if(faild) {
      stop("Did not find id file",path.fn,"; there must be a separate id file for each LRR/BAF input file")
    }
  }
 ## print(path.fn)
                                   ...
                                   snp.fields=NULL,HD=F,multi=F,n.cores=1,verbose=T)                       
{
 ...
 file.lens.sub <- round((file.lens.nh)/num.markers)
                             ...
                             f.snp="", f.samp="", verbose=T)
{
  ...
  bigMat <- get.big.matrix(des.fn,dir=dir)
...
add.color.to.snp.info <- function(snp.info,scheme=c("mono","alt","unique","hilite"),
  ...
  # mono = all col1, alternate = alternate between col1 and col2, unique = all different, hilite all col2 except 'hilite' chrm(s)
  must.use.package("genoset",T)
...
# prv(dir,snp.info.fn,snp.fn,anot)
...
make.fam.file <- function(sample.info,dir,out.fn="plink.fam")
{
  ...
  #Affection (0=unknown; 1=unaffected; 2=affected,"\n")
  dir <- validate.dir.for(dir,"cnv.qc")
  PED.FAM.TEMP <- sample.info[,rep(1,6)]
  PED.FAM.TEMP[,6] <- 1
                                      ...
                                      ndels=NULL, ndups=NULL, del.rate=0.4, dup.rate=0.18, QC.ON=T, pval=0.05)
{
  ...
  if(is.null(dim(ord.list))) { 
    if(length(ord.list)<1) {
      ord.list <- matrix("",nrow=0,ncol=2)
    } else {
      ord.list <- matrix(ord.list,ncol=2)
    }
  }
  #prv(lenz,ord.list,ss,tt)
  ...
  txto <- as.numeric(ord.list[,2])
  if(length(txto)>2) { print(textogram(txto)) }  # suppress histo if less than 3 CNVs
  if(QC.ON) {
...
force.good.plink <- function(fn,silent=FALSE) {
  FAM <- read.pedData(fn,silent=silent)
  colnames(FAM) <- c("family","sample","father","mother","sex","phenotype")
  FAM$sample <- rownames(FAM)
  FAM$father[is.na(FAM$father)] <- 0
  FAM$mother[is.na(FAM$mother)] <- 0
  rownames(FAM) <- NULL
  return(FAM)
}
                         ...
                         snp.info.fn="snpdata.map",anot="map3",build="hg18",autosomes.only=T,...)
{
  ...
  samp.qc.list <- doSampQC(dir=dir, subIDs.plink=subIDs.actual, plink=import.plink,
                           call.rate=callrate.samp.thr, snpMatLst=snpMatLst, 
                           het.lo=het.lo,het.hi=het.hi,
                           n.cores=l.cores, proc=2)
                              ...
                              lo.cut=c("LB",NA,"LB"),hi.cut=c("UB","UB","UB"),build="hg18",...)
{
  ...
  bigMat2 <- get.big.matrix(R.descr,dir=dir)
                          ...
                          dlrs=T,batch="plate",lrr.report=T,chr.ab.report=T,plate.report=T,cohort.pc.correct=F,add.int=F,n.cores=1,...)
{
 ...
 # sample.info <- validate.samp.info(sample.info,dir) #DEP file.spec.txt
  ...
  if(ngrp<2) { cohort.pc.correct <- FALSE }
  for (grp in 1:ngrp) {
  ...
  #prv(descr.list,descr.fn)
  new.descr <- exclude.combine.big.mats(descr.list,dir=dir) 
 ...
 ## sample.info$QCfail[rownames(sample.info) %in% get.all.samp.fails(dir)] <- 1
  ...
  crs <- samp.cr.summary(get.big.matrix(new.descr,dir=dir$big),print=T)
...
dat.to.big.matrix <- function(dir=NULL, grp=NA, snp.fn="snpNames.txt", sample.fn=NULL, 
...
lrr.dat.to.big.matrix <- function(dir=NULL,ngrp=1,import.LRR="LRR.dat",
                                    ...
                                    relative=F,...) 
{
  ...
  bigBAF <- get.big.matrix(BAF.fn, dir=dir$big)
  bigLRR <- get.big.matrix(LRR.fn, dir=dir$big)
    ...
    for (nn in 1:length(dir.sizes))
    {
      ...
      if(low.ram) {
        ...
        if(test) {
          ...
          bigBAF <- get.big.matrix(BAF.fn, dir=dir$big,verbose=F)
          bigLRR <- get.big.matrix(LRR.fn, dir=dir$big,verbose=F)
        }
      }
    }
  }
...
prepare.penncnv.markers <- function(snp.info,lrr.descr, baf.descr="BAFdescrFile", dir, build="hg18", n.cores=1)
{
  ...
  bigMat2 <- get.big.matrix(lrr.descr,dir=dir)
...
#' @param stagger numeric, number of seconds to leave between jobs (may prevent overworking
#' server HD for many simultaneous R/W operations near the start and ends of jobs).
#' @param max.conc integer, maximum number of concurrent jobs on the queue (e.g, if less
#' than length of bash.commands). Prevents sending too many jobs to the server. If value
#' is NA, will send all jobs to the server.
...
bash.qsub <- function(bash.commands, dir=getwd(), hrs.guess=NA, stagger=0, max.conc=NA,
  ...
  if(!is.numeric(stagger)) { stop("stagger must be numeric") }
  if(stagger>1000000) { stop("stagger was too large") }
  n.jobs.tot <- length(bash.commands)
  n.jobs.now <- min(c(n.jobs.tot,max.conc),na.rm=TRUE)
  if(n.jobs.now<n.jobs.tot) { jobs.left <- n.jobs.tot-n.jobs.now } else { jobs.left <- 0 }
  for (tt in 1:n.jobs.now) {  #length(bash.commands)) {
    # make the sh file
    # make the qsub call
    q.submit(penncmd=bash.commands[tt],tt,dest=dir,cluster.fn=cluster.fn,grid.name=grid.name,logpref=logpref) 
    wait(stagger,"s")
  }
  cat(" submitted",n.jobs.now,"jobs to",grid.name,"[qsub]\n")
  if(jobs.left>0) { cat(" ",jobs.left,"further jobs to schedule once the initial set begins to complete\n") }
  ...
  if(length(files.so.far)==0) {
    ...
    not.complete <- T
    kk <- proc.time()[3]
    done.now <- integer(0); no.act <- 0; notes.per.hr <- 2; hrs.so.far <- 0
    while(not.complete) {
      ...
      if(length(which.done)>length(done.now)) {
        ...
        if(jobs.left>0) {
          just.done <- length(which.done)-length(done.now)
          if(just.done>0) {
            jkl <- min(just.done,jobs.left)
            cat("submitting",jkl,"more", if(jkl==1) { "job" } else {"jobs"},"to the queue..\n")
            for (uu in 1:just.done) {  #length(bash.commands)) {
              tt <- tt+1
              if(tt %in% 1:length(bash.commands)) {
                q.submit(penncmd=bash.commands[tt],tt,dest=dir,cluster.fn=cluster.fn,grid.name=grid.name,logpref=logpref) 
                jobs.left <- jobs.left-1
              } else {
                # not a valid command number (perhaps about to finish)
              }
            }
          }
        }
...
## internal - previous version of function 
bash.qsub2 <- function(bash.commands, dir=getwd(), hrs.guess=NA,
         grid.name="all.q", cluster.fn="q.cmd", interval=60, logpref="X") {
  cat(" submitting bash commands to grid...\n")
  if(!is.na(hrs.guess)) {
    cat(" expect processing via the cluster to take roughly ",round(hrs.guess,2),"hrs ...\n",sep="")
...
delete.big.matrix.files <- function(bigMat,dir) {
  ...
  bm <- get.big.matrix(bigMat,dir=dir$big)
  ott <- describe(bm)
...
qscore.cnvs <- function(cnv.ranges,DT=NULL,file="all.ranges.pdf",dir="",pc.flank=5,snp.info=NULL,
  ...
  idz <- cnv.ranges$id; stz <- start(cnv.ranges); enz <- end(cnv.ranges); chrz <- chrm(cnv.ranges); wz <- width(cnv.ranges)
  ...
  if(all(c("lrr.file","baf.file","lrr.file.raw") %in% names(XX))) {
    ...
    if(!PREPOSTPC) {
      ...
      if(LRR) { lrr.file <- get.big.matrix(lrr.file,dir=dir$big) }
      if(BAF) { baf.file <- get.big.matrix(baf.file,dir=dir$big) }
    } else {
...
get.ratio.set <- function(ranged,ratio=5,bp=NA) {
  stz <- start(ranged)    ; enz <- end(ranged); chr <- chrm(ranged)
#  chr.lens <- get.chr.lens()

  ch <- chrnums.to.txt(chrNames(ranged))
...
calc.quality.stats <- function(DEL, bigBAF, bigPCC, snp.info, pr.acc=0.5, 
  ...
  if(do.chr) {
    ...
    chrDat <- big.extract.snp.ranges(chrSnps[as.numeric(chrm(DEL)),],samples=DEL$id,bigPCC)
  }
  #prv(DEL)
...
make.sample.info <- function(dir="",id.list=NULL,phenotype=NULL,plate=NULL,grp=NULL,sex=NULL,
  ...
  #prv(id.list,dir)
  id.list <- force.vec(id.list,dir=dir)
...
LRR.gc.correct <- function(dir,snp.info,bigLRR,pref="GC",write=F,add.means=T,n.cores=1,build="hg18")
{
  ...
  origMat <- get.big.matrix(bigLRR,dir=dir)
...
cnv.plot <- function(dir="",samples="",LRR=T,BAF=F,PREPOSTPC=F,n.pcs=NA,
  ...
  if(LRR) { 
    bigMat2 <- get.big.matrix(lrr.file,dir=dir$big) ;
    if(PREPOSTPC) { 
      bigMat1 <- get.big.matrix(lrr.file.raw,dir=dir$big)
  ...
  if(BAF) { 
    bigMat3 <- get.big.matrix(baf.file,dir=dir$big) 
...
get.dat.for.ranges <- function(ranges,dir,pcCor=T,BAF=F,snp.names=T,snp.pos=F,genome=F) {
  ...
  bigMat <- get.big.matrix(fn,dir=dir$big)
  snp.info <- read.snp.info(dir)
  snp.info <- snp.info[snp.info$QCfail==0,]
                          ...
                          ratio=10, build="hg18", dir="",scheme=c("mono","alt","unique","hilite"),...) 
{
 ...
 # print(head(snp.info))
  ...
  if(is(snp.info)[1]=="RangedData" & (all(c("QCfail","gindx") %in% colnames(snp.info)))) { 
    ...
    snp.info <- snp.info[(rownames(snp.info) %in% rownames(bigMat)),]
    if(!isGenomeOrder(as(snp.info,"GRanges"))) { warning("bigMat rows (snps) are not in genome order"); return(NULL) }
    #uv <- tolower(universe(snp.info)); if(length(uv)>0) { if(uv %in% paste("hg",16:20,sep="")) { build <- uv } }
  ...
  if(!whole.genome & all(!is.na(rng.mb))) {
    ...
    chr.x.labs <- NULL
    #print(rng.mb)
    LRR.mat <- LRR.mat[paste(chrm(snp.info))==paste(targ.chr.ref)]
...
count.cnvs <- function(cnv.hit.list,cnv.sample.map=NULL,by.phenotype=F,tot.cnvs=NA,
  ...
  if(!is.list(cnv.hit.list)) { 
    warning("cnv.hit.list should be a list, 1 element per CNV, each listing ids/regions hit, returning empty list")
    return("")
  }
...
do.CNV.all.overlaps.summary <- function(cnvResults,dir,comps=c(1:5),dbs=c(1:3),...) {
  ...
  if(!all(sapply(lapply(cnvResults,is),"[",1)=="RangedData")) { fail <- T }
  if(fail) { 
...
pheno.ratios.table <- function(dir,sum.table)
{
  ...
  if(all(sapply(lapply(sum.table,is),"[",1)!="matrix")) { stop("sum.table was not a list of matrices") }
    ...
    if(ret.obj) {
      if(is.file(out,dir)) {
        ...
        if(stdz(what) %in% stdz(mdt$bigs) & length(out[[1]])==1) {
          out <- get.big.matrix(out[[1]][1],dir=dir)  
        }
...
add.info.to.ranges <- function(dir,cnv.ranges,target="phenotype",info.file="pheno.lookup.txt") {
  ...
  if(!"id" %in% colnames(cnv.ranges)) { warning("cnv.ranges must have column 'id' to add phenotype data automatically"); return(cnv.ranges) }
  ...
  copy.from.aux <- function(src,targ) {
    if(file.exists(src)) {
      if(!file.exists(targ)) {
        ...
        print(length(readLines(src)));print(length(readLines(targ)))
    } }
  }
  if(!is.null(info.dir)) {
    if(file.spec!="") {
...
check.readiness <- function(dir=NULL,mode=2,snp.mode=1,penn.check=T,plink.check=T,plink.cmd="plink",
  ...
  if(any(!check.linux.install(c("sed","grep","zcat","seq","awk","cat")))) {
    if(mode==1) {
  ...
  if(all(!check.linux.install(c("getopt","getopts")))) {
    if(mode==1) {
      stop("Error: getopt/getopts bash commands both missing necessary when mode='scratch' - please install one of these\n")
    } else {
      warning("getopt/getopts bash commands are missing: should be ok because 'mode' is not 'scratch'")      
    }
  }
                     ...
                     rmv.low.plates=F,min.sites=10,rare.olp=0.5,rare.pc=0.01,rmv.bad.reg=T,settings=NULL) 
{
  ...
  orig.dir <- getwd()
  dont.make.fake <- TRUE  # whether to use custom family file if ped.file exists
...
#         }
  ...
  if(file.exists(ped.file)) { 
    own.fam.file <- dont.make.fake # if a ped file is provided, try to use it for any plink analyses
    if(plink.imp & own.fam.file) {
      std.ped.fn <- cat.path(dir$col,"snpdata.fam")
      cmm <- paste0("cp ",ped.file," ",std.ped.fn)
      cat("\nOption to use custom ped file for Plink SNP-QC in use, importing now\n")
      system(cmm)
      FAM <- force.good.plink(std.ped.fn,silent=TRUE) # convert if necessary into proper format for plink
      cat(" preview of custom ped-file",std.ped.fn," (as converted):\n")
      print(head(FAM))
      write.table(FAM,file=std.ped.fn,quote=F,col.names=T,sep="\t",row.names=FALSE)
      #write.table(FAM,file=paste0(std.ped.fn,2),quote=F,col.names=T,sep="\t",row.names=FALSE)
      #print(cmm)
      warning("NB: using custom plink ped file: ",ped.file,
              " rather than an automatically generated ped file (snpdata.fam)",
              ", the plumbCNV pipeline will fail if ped file format is incorrect")
    }
  ...
  if(snp.run.mode!="skip" & start.at<3) { 
    ...
    DT <- run.SNP.qc(DT=DT,import.plink=(snp.run.mode=="plink"),HD.mode=HD.mode,
                     ...
                     grp.cr.thr=grp.cr.thr,coverage.loss=coverage.loss,
