...
#function to make the nice graphs, given a set of starting params
...
FET <- function(case,ctrl,dir=NULL,sample.info=NULL,case.d=NA,cont.d=NA) {
  skip.c <- F
  ...
  for (cc in 1:length(case)) {
    ...
    if(length(case[cc])>0 & length(ctrl[cc])>0) { 
     fet[cc] <- fisher.test(x=matrix(c(case[cc],case.d,ctrl[cc],cont.d),nrow=2))$p.value
    } else {
      fet[cc] <- NA
    }
